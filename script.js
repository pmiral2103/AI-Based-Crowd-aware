            ...
            if (!this.isWall(x, y)) {
                if (this.isOnline && this.role === 'civilian') {
                    ...
                    if (this.role === 'firefighter') {
                        ...
                        // Mix of behaviors: 50% Rescue Fire, 50% Rescue Human
                        // User Request: Don't show exit path for firefighters.
                        const action = Math.random() > 0.5 ? 'rescue_fire' : 'rescue_civilian';
                        this.users.push({
                            x, y,
